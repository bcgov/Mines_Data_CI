...
try {
    ...
    # *** Begin: Logging New Artifacts Being Deployed ***
    Write-Host "=== New Artifacts Being Deployed ===" -ForegroundColor Cyan

    # Group resources by their type
    $groupedResources = $resources | Group-Object -Property type

    foreach ($group in $groupedResources) {
        # Print the resource type in Yellow
        Write-Host "`nResource Type: $($group.Name)" -ForegroundColor Yellow
        
        foreach ($resource in $group.Group) {
            # Extract the resource name based on your naming convention
            $resourceName = $resource.name.Substring(37, $resource.name.Length - 40) # Adjust indices if necessary
            
            # Print the resource name in Green
            Write-Host " - Name: $resourceName" -ForegroundColor Green
        }
    }

    Write-Host "`n====================================`n" -ForegroundColor Cyan
    # *** End: Logging New Artifacts Being Deployed ***
    ...
    if ($PreDeployment -eq $true) {
        ...
        if ($DeleteDeployment -eq $true) {
            ...
            # Retrieve all deployments in the specified resource group
            Write-Verbose "Fetching deployments in resource group: $ResourceGroupName"
            $deployments = Get-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName
        
            if ($deployments -eq $null) {
                Write-Warning "No deployments found in the resource group '$ResourceGroupName'. Skipping deletion."
                exit 0
            }
        
            Write-Verbose "Total deployments found: $($deployments.Count)"
        
            # Filter deployments based on naming patterns
            $deploymentsToConsider = $deployments | Where-Object { 
                $_.DeploymentName -like "ArmTemplate_master*" -or 
                $_.DeploymentName -like "ArmTemplateForFactory*" 
            } | Sort-Object -Property Timestamp -Descending
        
            Write-Verbose "Deployments matching criteria: $($deploymentsToConsider.Count)"
        
            # Check if any deployments match the criteria
            if ($deploymentsToConsider -and $deploymentsToConsider.Count -gt 0) {
                $deploymentName = $deploymentsToConsider[0].DeploymentName
                Write-Host "Deployment to be deleted: $deploymentName"
        
                # Retrieve operations for the specific deployment
                $deploymentOperations = Get-AzResourceGroupDeploymentOperation -DeploymentName $deploymentName -ResourceGroupName $ResourceGroupName
        
                # Filter operations targeting inner deployments
                $deploymentsToDelete = $deploymentOperations | Where-Object { 
                    $_.properties.targetResource.id -like "*Microsoft.Resources/deployments*" 
                }
        
                Write-Verbose "Inner deployments to delete: $($deploymentsToDelete.Count)"
        
                # Delete inner deployments first
                $deploymentsToDelete | ForEach-Object {
                    Write-Host "Deleting inner deployment: $($_.properties.targetResource.id)"
                    Remove-AzResourceGroupDeployment -Id $_.properties.targetResource.id -Force
                }
        
                # Delete the main deployment
                Write-Host "Deleting deployment: $deploymentName"
                Remove-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName -Name $deploymentName -Confirm:$false # updated due to error  used to be -Force#
            }
            else {
                Write-Warning "No deployments matching the criteria were found in the resource group '$ResourceGroupName'. Skipping deletion."
            }
        }
