...
|------|-------------|------|---------|:--------:|
| <a name="input_Default_Owners"></a> [Default\_Owners](#input\_Default\_Owners) | Additional owners to be added to the default owners. Must be passed as user object IDs from Azure AD / Entra ID. | `list(string)` | <pre>[<br>  "",<br>  "",<br>  "",<br>  ""<br>]</pre> | no |
...
| <a name="input_Owners"></a> [Owners](#input\_Owners) | The owner for specified capactities must be passed as a users object id from Azure AD / Entra ID defaults to Sebastian Hansen this must be the users upn | `list` | <pre>[<br>  ""<br>]</pre> | no |
...
| <a name="input_location"></a> [location](#input\_location) | Location of the Azure resources | `string` | `"WestUS2"` | no |
